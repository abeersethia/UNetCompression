"""
Direct Manifold Reconstruction - MLP Architecture

Pipeline:
1. Input: Hankel matrix of X component (B, delay_dim, window_len)
2. Encoder: Hankel → Compressed Manifold (B, 3, T)
3. Decoder: Compressed Manifold → Direct Time-Domain Signals (B, 3, T)
...
class DirectMLPEncoder(nn.Module):
    """MLP Encoder: Hankel → Compressed Manifold"""
    def __init__(self, input_d, input_l, output_t):
        ...
        output_size = 3 * output_t  # 3 components (X, Y, Z) × T timesteps
    ...
    def forward(self, x):
        ...
        batch_size = x.size(0)
        x_flat = x.view(batch_size, -1)  # (B, input_d*input_l)
        output_flat = self.mlp(x_flat)   # (B, 3*output_t)
        output = output_flat.view(batch_size, 3, self.output_t)  # (B, 3, T)
        return output

class DirectMLPDecoder(nn.Module):
    """MLP Decoder: Compressed Manifold → Direct Signals"""
    def __init__(self, input_t, output_t):
        super().__init__()
        self.input_t = input_t
        self.output_t = output_t
        
        input_size = 3 * input_t
        output_size = 3 * output_t
    ...
    def forward(self, x):
        ...
        batch_size = x.size(0)
        x_flat = x.view(batch_size, -1)  # (B, 3*input_t)
        output_flat = self.mlp(x_flat)   # (B, 3*output_t)
        output = output_flat.view(batch_size, 3, self.output_t)  # (B, 3, T)
        return output

class DirectManifoldMLPReconstructor:
    """Direct Manifold Reconstructor using MLP"""
    ...
    def __init__(self, window_len=512, delay_embedding_dim=10, stride=5, 
        ...
        self.original_signals = None  # Store original X, Y, Z
    ...
    def prepare_data(self, traj, t):
        ...
        self.original_signals = {'x': x, 'y': y, 'z': z}  # Store originals
    ...
    def train(self, max_epochs=150, base_noise_std=0.1, patience=25, verbose=True):
        ...
        print(f"\n=== TRAINING DIRECT MANIFOLD MLP ===")
        print(f"Architecture: MLP (Hankel → Direct Signal)")
    ...
    def reconstruct_manifold(self):
        ...
        print(f"\n=== RECONSTRUCTING DIRECT MANIFOLD (MLP) ===")
...
def main():
    """Test Direct Manifold MLP"""
    print("=== DIRECT MANIFOLD RECONSTRUCTION (MLP) ===")
    ...
    print(f"\n=== DIRECT MLP SUMMARY ===")
    print(f"✅ Architecture: Direct Manifold MLP")
