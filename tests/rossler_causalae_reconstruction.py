"""
Rössler Attractor X-Only Reconstruction using CausalAE
Reconstructs the full Rössler attractor from X component only using CausalAE architecture
"""
...
# Import CausalAE architecture and Rössler generator
...
def test_rossler_causalae_reconstruction():
    """Test CausalAE reconstruction of Rössler attractor from X-only input"""
    print("="*80)
    print("RÖSSLER ATTRACTOR X-ONLY RECONSTRUCTION - CAUSALAE")
    print("="*80)
    print("Reconstructing full Rössler attractor from X component only using CausalAE")
    print()
    ...
    print("🔄 Creating CausalAE reconstructor...")
    ...
    print("🔄 Preparing data and training CausalAE...")
    ...
    print("🔄 Creating Rössler CausalAE reconstruction visualization...")
    ...
    # 2. CausalAE Reconstructed Rössler (3D)
    ...
    ax2.set_ylabel('Y')
    ax2.set_zlabel('Z')
    ax2.set_title('CausalAE Reconstructed Rössler', fontsize=12, fontweight='bold')
    ax2.legend()
    ...
    ax4.plot(reconstructed_attractor[:, 0], reconstructed_attractor[:, 1], alpha=0.8, linewidth=1, color='red', label='CausalAE')
    ax4.set_xlabel('X')
    ax4.set_ylabel('Y')
    ...
    ax5.plot(t, reconstructed_attractor[:, 0], alpha=0.8, linewidth=1, color='red', label='CausalAE X')
    ax5.set_xlabel('Time')
    ...
    ax6.plot(t, reconstructed_attractor[:, 1], alpha=0.8, linewidth=1, color='red', label='CausalAE Y')
    ax6.set_xlabel('Time')
    ...
    ax7.plot(t, reconstructed_attractor[:, 2], alpha=0.8, linewidth=1, color='red', label='CausalAE Z')
    ax7.set_xlabel('Time')
    ...
    summary_text = f'''RÖSSLER ATTRACTOR X-ONLY RECONSTRUCTION - CAUSALAE:

ARCHITECTURE: CausalAE (Hankel → Direct Signal)
...
SUCCESS: ✅ Rössler attractor reconstructed from X-only input!'''
    ...
    plt.suptitle('Rössler Attractor X-Only Reconstruction - CausalAE\nFrom X Component to Full Attractor', 
                 fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.savefig('plots/rossler_causalae_reconstruction.png', dpi=300, bbox_inches='tight')
    print("✅ Rössler CausalAE reconstruction saved to: plots/rossler_causalae_reconstruction.png")
    
    # Print results
    print("\n" + "="*80)
    print("RÖSSLER CAUSALAE RECONSTRUCTION RESULTS")
    print("="*80)
    print(f"🏗️  Architecture: CausalAE")
    ...
    print(f"\n✅ SUCCESS: CausalAE successfully reconstructs Rössler attractor from X-only input!")
    print(f"🎯 CausalAE's temporal causality is well-suited for Rössler's dynamics.")
    
    plt.show()
    return reconstructor, original_attractor, reconstructed_attractor, metrics

if __name__ == "__main__":
    reconstructor, original_attractor, reconstructed_attractor, metrics = test_rossler_causalae_reconstruction()
