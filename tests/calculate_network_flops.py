...
def analyze_architecture_flops(ArchClass, name, input_shape=(1, 10, 512), verbose=False):
    ...
    try:
        ...
        decoder_input_shape = (1, 32, 256)  # Approximate latent shape
...
def main():
    ...
    print("="*80)
    print("NETWORK FLOPs CALCULATION")
    print("="*80)
    ...
    plt.tight_layout()
    plt.savefig('plots/network_flops_analysis.png', dpi=300, bbox_inches='tight')
    print("✅ FLOPs analysis saved to: plots/network_flops_analysis.png")
    ...
    # Save results to CSV
    df.to_csv('plots/flops_analysis_results.csv', index=False)
    print(f"📊 Results saved to: plots/flops_analysis_results.csv")
