"""
Rössler Attractor X-Only Reconstruction using MLP
Reconstructs the full Rössler attractor from X component only using MLP architecture
"""
...
# Import MLP architecture and Rössler generator
...
def test_rossler_mlp_reconstruction():
    """Test MLP reconstruction of Rössler attractor from X-only input"""
    print("="*80)
    print("RÖSSLER ATTRACTOR X-ONLY RECONSTRUCTION - MLP")
    print("="*80)
    print("Reconstructing full Rössler attractor from X component only using MLP")
    print()
    ...
    print("🔄 Creating MLP reconstructor...")
    ...
    print("🔄 Preparing data and training MLP...")
    ...
    print("🔄 Creating Rössler MLP reconstruction visualization...")
    ...
    # 2. MLP Reconstructed Rössler (3D)
    ...
    ax2.set_ylabel('Y')
    ax2.set_zlabel('Z')
    ax2.set_title('MLP Reconstructed Rössler', fontsize=12, fontweight='bold')
    ax2.legend()
    ...
    ax4.plot(reconstructed_attractor[:, 0], reconstructed_attractor[:, 1], alpha=0.8, linewidth=1, color='red', label='MLP')
    ax4.set_xlabel('X')
    ax4.set_ylabel('Y')
    ...
    ax5.plot(t, reconstructed_attractor[:, 0], alpha=0.8, linewidth=1, color='red', label='MLP X')
    ax5.set_xlabel('Time')
    ...
    ax6.plot(t, reconstructed_attractor[:, 1], alpha=0.8, linewidth=1, color='red', label='MLP Y')
    ax6.set_xlabel('Time')
    ...
    ax7.plot(t, reconstructed_attractor[:, 2], alpha=0.8, linewidth=1, color='red', label='MLP Z')
    ax7.set_xlabel('Time')
    ...
    summary_text = f'''RÖSSLER ATTRACTOR X-ONLY RECONSTRUCTION - MLP:

ARCHITECTURE: MLP (Hankel → Direct Signal)
...
- Good test for X-only reconstruction
...
SUCCESS: ✅ Rössler attractor reconstructed from X-only input!'''
    ...
    plt.suptitle('Rössler Attractor X-Only Reconstruction - MLP\nFrom X Component to Full Attractor', 
                 fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.savefig('plots/rossler_mlp_reconstruction.png', dpi=300, bbox_inches='tight')
    print("✅ Rössler MLP reconstruction saved to: plots/rossler_mlp_reconstruction.png")
    
    # Print results
    print("\n" + "="*80)
    print("RÖSSLER MLP RECONSTRUCTION RESULTS")
    print("="*80)
    print(f"🏗️  Architecture: MLP")
    ...
    print(f"\n✅ SUCCESS: MLP successfully reconstructs Rössler attractor from X-only input!")
    print(f"🎯 The Rössler attractor (simpler than Lorenz) is well-suited for X-only reconstruction.")
    
    plt.show()
    return reconstructor, original_attractor, reconstructed_attractor, metrics

if __name__ == "__main__":
    reconstructor, original_attractor, reconstructed_attractor, metrics = test_rossler_mlp_reconstruction()
