...
Shows the reconstruction quality of ModUNet architecture
"""
...
def main():
    """Create ModUNet reconstruction visualization"""
    print("="*80)
    print("MODUNET RECONSTRUCTION VISUALIZATION")
    print("="*80)
    print("Modular U-Net Architecture for X-Only Manifold Reconstruction")
    print()
    ...
    # Create ModUNet reconstructor
    print("🔄 Creating ModUNet reconstructor...")
    reconstructor = DirectManifoldModUNetReconstructor(
        window_len=512,
    ...
    print("🔄 Training ModUNet with 100 epochs...")
    with tqdm(total=100, desc="Training Progress", unit="epoch") as pbar:
        training_history = reconstructor.train(max_epochs=100, verbose=False)
    ...
    # 2. ModUNet Reconstructed Attractor (3D)
    ...
    ax2.plot(reconstructed_attractor[:, 0], reconstructed_attractor[:, 1], reconstructed_attractor[:, 2], 
             alpha=0.8, linewidth=1, color='red', label='ModUNet Reconstructed')
    ...
    ax2.set_ylabel('Y')
    ax2.set_zlabel('Z')
    ax2.set_title('ModUNet Reconstructed\n(100 Epochs)', fontsize=12, fontweight='bold')
    ax2.legend()
    ...
    ax3.set_ylabel('Y')
    ax3.set_zlabel('Z')
    ax3.set_title('Overlay Comparison\n(Original vs ModUNet)', fontsize=12, fontweight='bold')
    ax3.legend()
    
    # 4. X-Y Projection
    ax4 = fig.add_subplot(3, 4, 4)
    ax4.plot(traj[:, 0], traj[:, 1], alpha=0.8, linewidth=1, color='blue', label='Original')
    ax4.plot(reconstructed_attractor[:, 0], reconstructed_attractor[:, 1], alpha=0.8, linewidth=1, color='red', label='ModUNet')
    ax4.set_xlabel('X')
    ax4.set_ylabel('Y')
    ...
    ax5.plot(reconstructed_attractor[:, 1], reconstructed_attractor[:, 2], alpha=0.8, linewidth=1, color='red', label='ModUNet')
    ax5.set_xlabel('Y')
    ax5.set_ylabel('Z')
    ...
    ax6.plot(reconstructed_attractor[:, 0], reconstructed_attractor[:, 2], alpha=0.8, linewidth=1, color='red', label='ModUNet')
    ax6.set_xlabel('X')
    ax6.set_ylabel('Z')
    ...
    ax7.plot(time_axis, reconstructed_attractor[:, 0], alpha=0.8, linewidth=1, color='red', label='ModUNet X')
    ax7.set_xlabel('Time')
    ...
    ax8.plot(time_axis, reconstructed_attractor[:, 1], alpha=0.8, linewidth=1, color='red', label='ModUNet Y')
    ax8.set_xlabel('Time')
    ...
    ax9.plot(time_axis, reconstructed_attractor[:, 2], alpha=0.8, linewidth=1, color='red', label='ModUNet Z')
    ax9.set_xlabel('Time')
    ...
    metrics_text = f'''MODUNET RECONSTRUCTION RESULTS:

ARCHITECTURE: Modular U-Net
CONFIGURATION: Skip connections + Residual blocks
...
2. ModUNet: Modular blocks + Skip connections
3. Output: Direct signals (B, 3, 512)
4. Result: Good reconstruction quality!'''

    ax12.text(0.05, 0.95, metrics_text, transform=ax12.transAxes, fontsize=10,
              verticalalignment='top', fontfamily='monospace',
              bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8))
    
    plt.suptitle('ModUNet Reconstruction Visualization\nModular U-Net Architecture', 
                 fontsize=20, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.savefig('plots/modunet_reconstruction.png', dpi=300, bbox_inches='tight')
    print("✅ ModUNet reconstruction saved to: plots/modunet_reconstruction.png")
    
    # Print final results
    print("\n" + "="*80)
    print("MODUNET RECONSTRUCTION RESULTS")
    print("="*80)
    print(f"🏗️  Architecture: Modular U-Net")
