"""
Rössler Attractor X-Only Reconstruction using LSTM
Reconstructs the full Rössler attractor from X component only using LSTM architecture
"""
...
# Import LSTM architecture and Rössler generator
...
def test_rossler_lstm_reconstruction():
    """Test LSTM reconstruction of Rössler attractor from X-only input"""
    print("="*80)
    print("RÖSSLER ATTRACTOR X-ONLY RECONSTRUCTION - LSTM")
    print("="*80)
    print("Reconstructing full Rössler attractor from X component only using LSTM")
    print()
    ...
    print("🔄 Creating LSTM reconstructor...")
    ...
    print("🔄 Preparing data and training LSTM...")
    ...
    print("🔄 Creating Rössler LSTM reconstruction visualization...")
    ...
    # 2. LSTM Reconstructed Rössler (3D)
    ...
    ax2.set_ylabel('Y')
    ax2.set_zlabel('Z')
    ax2.set_title('LSTM Reconstructed Rössler', fontsize=12, fontweight='bold')
    ax2.legend()
    ...
    ax4.plot(reconstructed_attractor[:, 0], reconstructed_attractor[:, 1], alpha=0.8, linewidth=1, color='red', label='LSTM')
    ax4.set_xlabel('X')
    ax4.set_ylabel('Y')
    ...
    ax5.plot(t, reconstructed_attractor[:, 0], alpha=0.8, linewidth=1, color='red', label='LSTM X')
    ax5.set_xlabel('Time')
    ...
    ax6.plot(t, reconstructed_attractor[:, 1], alpha=0.8, linewidth=1, color='red', label='LSTM Y')
    ax6.set_xlabel('Time')
    ...
    ax7.plot(t, reconstructed_attractor[:, 2], alpha=0.8, linewidth=1, color='red', label='LSTM Z')
    ax7.set_xlabel('Time')
    ...
    summary_text = f'''RÖSSLER ATTRACTOR X-ONLY RECONSTRUCTION - LSTM:

ARCHITECTURE: LSTM (Hankel → Direct Signal)
...
- Good test for temporal modeling
...
SUCCESS: ✅ Rössler attractor reconstructed from X-only input!'''
    ...
    plt.suptitle('Rössler Attractor X-Only Reconstruction - LSTM\nFrom X Component to Full Attractor', 
                 fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.savefig('plots/rossler_lstm_reconstruction.png', dpi=300, bbox_inches='tight')
    print("✅ Rössler LSTM reconstruction saved to: plots/rossler_lstm_reconstruction.png")
    
    # Print results
    print("\n" + "="*80)
    print("RÖSSLER LSTM RECONSTRUCTION RESULTS")
    print("="*80)
    print(f"🏗️  Architecture: LSTM")
    ...
    print(f"\n✅ SUCCESS: LSTM successfully reconstructs Rössler attractor from X-only input!")
    print(f"🎯 LSTM's temporal modeling is well-suited for Rössler's simpler dynamics.")
    
    plt.show()
    return reconstructor, original_attractor, reconstructed_attractor, metrics

if __name__ == "__main__":
    reconstructor, original_attractor, reconstructed_attractor, metrics = test_rossler_lstm_reconstruction()
